...
public class StatsDataDao {
	...
	//10..나라별가장선호하는영화
	public List<Map<String,Object>> mostFilmByCountry(){
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		//DB 자원 준비
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		conn = DBUtil.getConnection();
		String sql ="SELECT t2.*, t3.title "
				+ "FROM (SELECT t1.country, MAX(t1.cnt) cnt "
				+ "      FROM (SELECT co.country country,f.title filmTitle, COUNT(*) cnt "
				+ "            FROM country co "
				+ "               INNER JOIN city ci "
				...
				+ "            GROUP BY co.country, f.title) t1 "
				+ "      GROUP BY t1.country) t2 "
				+ "      INNER JOIN (SELECT co.country country,f.title title, COUNT(*) cnt "
				+ "            FROM country co "
				+ "               INNER JOIN city ci "
				+ "               ON ci.country_id = co.country_id "
				+ "               INNER JOIN address ad "
				...
				+ "               INNER JOIN film f "
				+ "               ON i.film_id = f.film_id "
				+ "            GROUP BY co.country, f.title) t3 "
				+ "      ON t2.country = t3.country AND t2.cnt = t3.cnt"
				+ "		LIMIT 0,20 ";
		try {
			stmt = conn.prepareStatement(sql);
			rs = stmt.executeQuery();
